...
use crate::{testing::wait_for, topic::TopicId};

pub async fn introduce_and_wait(networks: impl IntoIterator<Item = &Network<TopicId>>) {
...
pub async fn introduce(networks: impl IntoIterator<Item = &Network<TopicId>>) {
    let networks = networks.into_iter().collect::<Vec<_>>();
